...
try {
    ...
    // LOGIN ENDPOINT - handle login action
    elseif (isset($_GET['action']) && $_GET['action'] === 'login' ||
           strpos($path, 'auth/login') !== false) {
        handleLogin($db, $jsonData);
    }
 ...
 */
function handleLogin(PDO $db, array $data): void {
    // Validate required fields
    ...
    try {
        ...
        $business = new Business($db);
        $hasBusiness = $business->findByUserId($user->user_id);
        $businessData = $hasBusiness ? $business->toArray() : null;
